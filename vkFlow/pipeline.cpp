...
namespace backend {
	...
	int Pipeline::create(const char* _name, const Option& opt, const std::vector<vk_specialization_type>& specializations, int binding_count, int push_constant_count)
	{
		...
		if (opt.use_fp16_arithmetic) { name += "_fp16a"; }
		else if (opt.use_fp16_storage) { name += "_fp16s"; }
		else if (opt.use_fp16_packed) {	name += "_fp16p"; }
	...
	void Pipeline::set_optimal_local_size_xyz(int w, int h, int c)
	{
		...
		{
			local_size_z = std::min<uint32_t>((uint32_t)128, vkdev->info.max_workgroup_size[2]);
		}
		...
		if (h > 0 && w > 0)
		{
			...
			uint32_t local_size_y_prefer = std::min<uint32_t>((uint32_t)128, vkdev->info.max_workgroup_size[1]);
			while (local_size_y < local_size_y_prefer)
			{
				local_size_y_prefer /= 2;
			}

			uint32_t local_size_x_prefer = std::min<uint32_t>((uint32_t)128, vkdev->info.max_workgroup_size[0]);
		...
		{
			local_size_y = std::min<uint32_t>(max_local_size_xy, vkdev->info.max_workgroup_size[1]);
			...
			local_size_x = std::min<uint32_t>(max_local_size_x, vkdev->info.max_workgroup_size[0]);
		}
		else if (w > 0)
		{
			local_size_x = std::min<uint32_t>(max_local_size_xy, vkdev->info.max_workgroup_size[0]);
			...
			local_size_y = std::min<uint32_t>(max_local_size_y, vkdev->info.max_workgroup_size[1]);
		}
	}
