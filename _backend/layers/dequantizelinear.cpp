...
namespace layers {    
    ...
    void DequantizeLinear::forward(){ 
        program->operator()({2, 1}, *_SHAPES, *backend::tensor_dict[m_x_i]->data, *backend::tensor_dict[m_x_scale_i]->data, *backend::tensor_dict[m_x_zero_point_i]->data, *backend::tensor_dict[m_y_o]->data);
        //program->run();
    }
