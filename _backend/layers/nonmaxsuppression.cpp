...
namespace layers {    
    ...
    void NonMaxSuppression::bind(std::string _boxes_i, std::string _scores_i, std::string _max_output_boxes_per_class_i, std::string _iou_threshold_i, std::string _score_threshold_i, std::string _selected_indices_o){
		...
		binding.boxes_i = backend::tensor_dict[boxes_i]->shape();
  		binding.scores_i = backend::tensor_dict[scores_i]->shape();
  		binding.max_output_boxes_per_class_i = backend::tensor_dict[max_output_boxes_per_class_i]->shape();
  		binding.iou_threshold_i = backend::tensor_dict[iou_threshold_i]->shape();
  		binding.score_threshold_i = backend::tensor_dict[score_threshold_i]->shape();
 
		binding.selected_indices_o = backend::tensor_dict[selected_indices_o]->shape();
    ...
    void NonMaxSuppression::build(){        
        ...
        program->bind(binding, *backend::tensor_dict[boxes_i]->data(), *backend::tensor_dict[scores_i]->data(), *backend::tensor_dict[max_output_boxes_per_class_i]->data(), *backend::tensor_dict[iou_threshold_i]->data(), *backend::tensor_dict[score_threshold_i]->data(), *backend::tensor_dict[selected_indices_o]->data());
    }
