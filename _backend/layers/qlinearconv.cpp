...
namespace layers {    
    ...
    void QLinearConv::build(){     
        ...
        (*program)({2, 1}, *_SHAPES, *backend::tensor_dict[m_x_i]->data, *backend::tensor_dict[m_x_scale_i]->data, *backend::tensor_dict[m_x_zero_point_i]->data, *backend::tensor_dict[m_w_i]->data, *backend::tensor_dict[m_w_scale_i]->data, *backend::tensor_dict[m_w_zero_point_i]->data, *backend::tensor_dict[m_y_scale_i]->data, *backend::tensor_dict[m_y_zero_point_i]->data, *backend::tensor_dict[m_B_i]->data, *backend::tensor_dict[m_y_o]->data);       
    }
