...
namespace backend {    
    OneHot::OneHot(std::string n, std::vector<std::string> i, std::vector<std::string> o, std::map<std::string, std::vector<std::string>> a) : Layer(n, i, o, a) {            
        program = new vuh::Program<Specs, Params>(*_get_device(), std::string(file_path + "/shaders/bin/onehot.spv").c_str());
        ...
        program->bind({axis, tensor_dict[indices_input]->shape(), tensor_dict[depth_input]->shape(), tensor_dict[values_input]->shape(), tensor_dict[output_output]->shape()} 
                        ...
                        , tensor_dict[indices_input], tensor_dict[depth_input], tensor_dict[values_input], tensor_dict[output_output] );
    }
