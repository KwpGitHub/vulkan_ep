...
namespace layers {    
    ...
    void OneHot::forward(){ 
        program->operator()({2, 1}, *_SHAPES, *backend::tensor_dict[m_indices_i]->data, *backend::tensor_dict[m_depth_i]->data, *backend::tensor_dict[m_values_i]->data, *backend::tensor_dict[m_output_o]->data);
        //program->run();
    }
