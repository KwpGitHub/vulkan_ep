...
namespace layers {    
    ...
    void InstanceNormalization::build(){     
        ...
        program->bind({2, 1}, *_SHAPES, *backend::tensor_dict[m_input_i]->data, *backend::tensor_dict[m_scale_i]->data, *backend::tensor_dict[m_B_i]->data, *backend::tensor_dict[m_output_o]->data);
    }
