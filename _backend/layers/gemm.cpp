...
namespace layers {    
    ...
    void Gemm::forward(){ 
        program->operator()({2, 1}, *_SHAPES, *backend::tensor_dict[m_A_i]->data, *backend::tensor_dict[m_B_i]->data, *backend::tensor_dict[m_C_i]->data, *backend::tensor_dict[m_Y_o]->data);
        //program->run();
    }
