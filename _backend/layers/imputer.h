...
namespace backend {
    ...
    public:
		...
		Shape_t imputed_value_floats_s;
        //input
        std::string X_input;
...
namespace backend {    
    Imputer::Imputer(std::string n, std::vector<std::string> i, std::vector<std::string> o, std::map<std::string, std::vector<std::string>> a) : Layer(n, i, o, a) {            
        program = new vuh::Program<Specs, Params>(*_get_device(), std::string(file_path + "/shaders/bin/imputer.spv").c_str());
        program->grid(1024/PROCESSKERNEL_SIZE, 1024/PROCESSKERNEL_SIZE, 64/PROCESSKERNEL_SIZE);
        program->spec(64,64,64);
        program->bind({imputed_value_int64s, replaced_value_float, replaced_value_int64, imputed_value_floats_s, tensor_dict[X_input]->shape(), tensor_dict[Y_output]->shape()} 
