...
namespace layers {    
    ...
    void Reshape::bind(std::string _data_i, std::string _shape_i, std::string _reshaped_o){
		...
		binding.data_i = backend::tensor_dict[data_i]->shape();
  		binding.shape_i = backend::tensor_dict[shape_i]->shape();
 
		binding.reshaped_o = backend::tensor_dict[reshaped_o]->shape();
    ...
    void Reshape::build(){        
        ...
        program->bind(binding, *backend::tensor_dict[data_i]->data(), *backend::tensor_dict[shape_i]->data(), *backend::tensor_dict[reshaped_o]->data());
    }
