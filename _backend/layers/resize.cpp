...
namespace layers {    
    ...
    void Resize::bind(std::string _X_i, std::string _scales_i, std::string _Y_o){    
		...
		SHAPES.push_back(backend::tensor_dict[X_i]->shape());
  		SHAPES.push_back(backend::tensor_dict[scales_i]->shape());
    ...
    void Resize::build(){     
        ...
        program->bind({128, 0.1f}, *_SHAPES, *backend::tensor_dict[X_i]->data, *backend::tensor_dict[scales_i]->data, *backend::tensor_dict[Y_o]->data);
    }
