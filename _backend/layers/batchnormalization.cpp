...
namespace layers {
	...
	void BatchNormalization::build() {
		...
		(*program)({ 2, 1, m_epsilon }, *_SHAPES, *backend::tensor_dict[m_X_i]->data, *backend::tensor_dict[m_scale_i]->data, *backend::tensor_dict[m_B_i]->data, *backend::tensor_dict[m_mean_i]->data, *backend::tensor_dict[m_var_i]->data, *backend::tensor_dict[m_Y_o]->data, *backend::tensor_dict[m_mean_o]->data, *backend::tensor_dict[m_var_o]->data, *backend::tensor_dict[m_saved_mean_o]->data, *backend::tensor_dict[m_saved_var_o]->data);
	}
