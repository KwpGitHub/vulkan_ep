...
namespace layers {    
    ...
    void ConvTranspose::init( std::string _auto_pad,  std::vector<int> _dilations,  int _group,  std::vector<int> _kernel_shape,  std::vector<int> _output_padding,  std::vector<int> _output_shape,  std::vector<int> _pads,  std::vector<int> _strides) {      
		 m_auto_pad = _auto_pad; 
 		 ...
 		 m_output_padding = _output_padding; 
 		 m_output_shape = _output_shape; 
 		 m_pads = _pads; 
 		 m_strides = _strides; 
    ...
    void ConvTranspose::forward(){ 
        program->operator()({2, 1}, *_SHAPES, *backend::tensor_dict[m_X_i]->data, *backend::tensor_dict[m_W_i]->data, *backend::tensor_dict[m_B_i]->data, *backend::tensor_dict[m_Y_o]->data);
        //program->run();
    }
