...
namespace layers {    
    ...
    void RoiAlign::forward(){ 
        program->operator()({2, 1}, *_SHAPES, *backend::tensor_dict[m_X_i]->data, *backend::tensor_dict[m_rois_i]->data, *backend::tensor_dict[m_batch_indices_i]->data, *backend::tensor_dict[m_Y_o]->data);
        //program->run();
    }
