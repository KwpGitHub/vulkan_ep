...
namespace backend {    
    ReverseSequence::ReverseSequence(std::string n, std::vector<std::string> i, std::vector<std::string> o, std::map<std::string, std::vector<std::string>> a) : Layer(n, i, o, a) {            
        program = new vuh::Program<Specs, Params>(*_get_device(), std::string(file_path + "/shaders/bin/reversesequence.spv").c_str());
        program->grid(1024/PROCESSKERNEL_SIZE, 1024/PROCESSKERNEL_SIZE, 64/PROCESSKERNEL_SIZE);
        program->spec(64,64,64);
        program->bind({batch_axis, time_axis, tensor_dict[input_input]->shape(), tensor_dict[sequence_lens_input]->shape(), tensor_dict[Y_output]->shape()} 
                        
                        , tensor_dict[input_input], tensor_dict[sequence_lens_input], tensor_dict[Y_output] );
    }
