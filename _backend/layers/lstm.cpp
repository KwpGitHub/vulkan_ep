...
namespace layers {    
    ...
    void LSTM::build(){     
        ...
        program->bind({2, 1}, *_SHAPES, *backend::tensor_dict[m_X_i]->data, *backend::tensor_dict[m_W_i]->data, *backend::tensor_dict[m_R_i]->data, *backend::tensor_dict[m_B_i]->data, *backend::tensor_dict[m_sequence_lens_i]->data, *backend::tensor_dict[m_initial_h_i]->data, *backend::tensor_dict[m_initial_c_i]->data, *backend::tensor_dict[m_P_i]->data, *backend::tensor_dict[m_Y_o]->data, *backend::tensor_dict[m_Y_h_o]->data, *backend::tensor_dict[m_Y_c_o]->data);
    }
