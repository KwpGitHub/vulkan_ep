...
namespace layers {    
    ...
    void QLinearMatMul::bind(std::string _a_i, std::string _a_scale_i, std::string _a_zero_point_i, std::string _b_i, std::string _b_scale_i, std::string _b_zero_point_i, std::string _y_scale_i, std::string _y_zero_point_i, std::string _y_o){
		...
		binding.a_i = backend::tensor_dict[a_i]->shape();
  		binding.a_scale_i = backend::tensor_dict[a_scale_i]->shape();
  		binding.a_zero_point_i = backend::tensor_dict[a_zero_point_i]->shape();
  		binding.b_i = backend::tensor_dict[b_i]->shape();
  		binding.b_scale_i = backend::tensor_dict[b_scale_i]->shape();
    ...
    void QLinearMatMul::build(){        
        ...
        program->bind(binding, *backend::tensor_dict[a_i]->data(), *backend::tensor_dict[a_scale_i]->data(), *backend::tensor_dict[a_zero_point_i]->data(), *backend::tensor_dict[b_i]->data(), *backend::tensor_dict[b_scale_i]->data(), *backend::tensor_dict[b_zero_point_i]->data(), *backend::tensor_dict[y_scale_i]->data(), *backend::tensor_dict[y_zero_point_i]->data(), *backend::tensor_dict[y_o]->data());
    }
