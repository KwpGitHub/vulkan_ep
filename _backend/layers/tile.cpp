...
namespace layers {    
    ...
    void Tile::bind(std::string _input_i, std::string _repeats_i, std::string _output_o){    
        m_input_i = _input_i; m_repeats_i = _repeats_i; m_output_o = _output_o;        
		SHAPES.push_back(backend::tensor_dict[m_input_i]->shape());
  		SHAPES.push_back(backend::tensor_dict[m_repeats_i]->shape());
    ...
    void Tile::forward(){ 
        program->operator()({2, 1}, *_SHAPES, *backend::tensor_dict[m_input_i]->data, *backend::tensor_dict[m_repeats_i]->data, *backend::tensor_dict[m_output_o]->data);
        //program->run();
    }
