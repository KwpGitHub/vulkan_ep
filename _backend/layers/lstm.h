...
namespace backend {    
    LSTM::LSTM(std::string n, std::vector<std::string> i, std::vector<std::string> o, std::map<std::string, std::vector<std::string>> a) : Layer(n, i, o, a) {            
        program = new vuh::Program<Specs, Params>(*_get_device(), std::string(file_path + "/shaders/bin/lstm.spv").c_str());
        program->grid(1024/PROCESSKERNEL_SIZE, 1024/PROCESSKERNEL_SIZE, 64/PROCESSKERNEL_SIZE);
        program->spec(64,64,64);
        program->bind({clip, direction, hidden_size, input_forget, activation_alpha_s, activation_beta_s, activations_s, tensor_dict[X_input]->shape(), tensor_dict[W_input]->shape(), tensor_dict[R_input]->shape(), tensor_dict[B_input_o]->shape(), tensor_dict[sequence_lens_input_o]->shape(), tensor_dict[initial_h_input_o]->shape(), tensor_dict[initial_c_input_o]->shape(), tensor_dict[P_input_o]->shape(), tensor_dict[Y_output_o]->shape(), tensor_dict[Y_h_output_o]->shape(), tensor_dict[Y_c_output_o]->shape()} 
                        , *activation_alpha, *activation_beta, *activations
                        , tensor_dict[X_input], tensor_dict[W_input], tensor_dict[R_input], tensor_dict[B_input_o], tensor_dict[sequence_lens_input_o], tensor_dict[initial_h_input_o], tensor_dict[initial_c_input_o], tensor_dict[P_input_o], tensor_dict[Y_output_o], tensor_dict[Y_h_output_o], tensor_dict[Y_c_output_o] );
    }
