...
namespace layers {    
    ...
    void MatMulInteger::forward(){ 
        program->operator()({2, 1}, *_SHAPES, *backend::tensor_dict[m_A_i]->data, *backend::tensor_dict[m_B_i]->data, *backend::tensor_dict[m_a_zero_point_i]->data, *backend::tensor_dict[m_b_zero_point_i]->data, *backend::tensor_dict[m_Y_o]->data);
        //program->run();
    }
