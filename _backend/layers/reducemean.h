...
namespace backend {   
    ...
    public:
        ReduceMean(std::string n, Shape_t axes, int keepdims);
...
namespace backend {    
    ...
    ReduceMean::ReduceMean(std::string n, Shape_t axes, int keepdims) : Layer(n) { }
    ...
    void ReduceMean::call(std::string data_input, std::string reduced_output){       
...
namespace backend {
    PYBIND11_MODULE(_backend, m) {
        py::class_<ReduceMean, Layer>(m, "ReduceMean")
            ...
            .def("init", &ReduceMean::init)
            .def("call", (void (ReduceMean::*) (std::string, std::string)) &ReduceMean::call);
    }
}
