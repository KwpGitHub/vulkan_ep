...
namespace backend {    
    GRU::GRU(std::string n, std::vector<std::string> i, std::vector<std::string> o, std::map<std::string, std::vector<std::string>> a) : Layer(n, i, o, a) {            
            ...
            program->bind({clip, direction, hidden_size, linear_before_reset, tensor_dict[X_input]->shape(), tensor_dict[W_input]->shape(), tensor_dict[R_input]->shape(), tensor_dict[B_input_o]->shape(), tensor_dict[sequence_lens_input_o]->shape(), tensor_dict[initial_h_input_o]->shape(), tensor_dict[Y_output_o]->shape(), tensor_dict[Y_h_output_o]->shape()}, 
