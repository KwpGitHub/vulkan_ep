...
namespace layers {    
    ...
    void Where::forward(){ 
        program->operator()({2, 1}, *_SHAPES, *backend::tensor_dict[m_condition_i]->data, *backend::tensor_dict[m_X_i]->data, *backend::tensor_dict[m_Y_i]->data, *backend::tensor_dict[m_output_o]->data);
        //program->run();
    }
