...
namespace backend {    
    ...
    ReduceLogSumExp::ReduceLogSumExp(std::string n, Shape_t axes, int keepdims) : Layer(n) { }
    ...
    void ReduceLogSumExp::call(std::string data_input, std::string reduced_output){       
        program = new vuh::Program<Specs, binding_descriptor>(*_get_device(), std::string(file_path + std::string("/shaders/bin/reducelogsumexp.spv")).c_str());
...
namespace backend {
    PYBIND11_MODULE(_backend, m) {
        py::class_<ReduceLogSumExp, Layer>(m, "ReduceLogSumExp")
            ...
            .def("init", &ReduceLogSumExp::init)
            .def("call", (void (ReduceLogSumExp::*) (std::string, std::string)) &ReduceLogSumExp::call);
    }
}
