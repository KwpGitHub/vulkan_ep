...
namespace backend {
    class MeanVarianceNormalization : public Layer {
        struct Params{Shape_t X_t; Shape_t Y_t; Shape_t axes_t;
        };
        ...
        //parameter 
        Shape_t X_t; Shape_t Y_t; Shape_t axes_t;

    public:
        MeanVarianceNormalization(std::string n, std::vector<std::string> i, std::vector<std::string> o, std::map<std::string, std::vector<std::string>> a): Layer(n, i, o, a) {
            X = i[0];
            Y = o[0];
            ...
            program->bind({X_t, Y_t, axes_t }, tensor_dict[Y], tensor_dict[X]);
