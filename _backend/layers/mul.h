...
namespace backend {    
    ...
    void Mul::call(std::string A_input, std::string B_input, std::string C_output){       
...
namespace backend {
    PYBIND11_MODULE(_backend, m) {
        py::class_<Mul, Layer>(m, "Mul")
            ...
            .def("init", &Mul::init)
            .def("call", (void (Mul::*) (std::string, std::string, std::string)) &Mul::call);
    }
}
