...
namespace kernel {
	...
	void layer::createPipeline(size_t push_constants_size, VkSpecializationInfo* specialization_info) {
		...
		push_constant_ranges[0].size = (uint32_t)push_constants_size;
	...
	void layer::recordCommandBuffer(void* push_constants, size_t push_constants_size) {
		...
		if (push_constants)
			vkCmdPushConstants(m_cmd_buffer, m_pipeline_layout, VK_SHADER_STAGE_COMPUTE_BIT, 0, (uint32_t)push_constants_size, push_constants);
